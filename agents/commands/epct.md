...
### 4b. Visual confirmation (MCP Chrome DevTools)
- Start the dev server (`pnpm dev` or equivalent) if not already running
- Use MCP Chrome DevTools to navigate to the affected page(s)
- Take a screenshot and verify the visual result matches expectations
- Check key interactions (clicks, navigation, form submissions) if relevant
- Report findings to the user with the screenshot
- **STAY IN SCOPE**: Only check pages/components you changed
