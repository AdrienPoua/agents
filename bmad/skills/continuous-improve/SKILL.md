...
**Plan (discovery + priority debate — edit nothing, commit nothing).** This stage *replaces* hardcoded project knowledge.
...
- **Decide the subject through a Product Manager ↔ Senior Developer debate**, never a lone "highest-impact" call — that biases to whatever ships easiest (usually tests) and starves features/UX/refactors. The two voices weigh user/product value against technical health and agree on ONE subject for this run — feature, refactor, cleanup, accessibility, perf, tests, as the case genuinely merits, rotating across runs. `$ARGUMENTS` biases the debate if given. Full mechanism, reconciliation criteria, the broadened sense of "verifiable", and the degraded path live in `references/planning-debate.md` — load it and follow it. The result is a concrete atomic plan (steps, files, claims to verify, verify command); if both voices conclude nothing is genuinely worth doing → clean **no-op**.

**Execute (on the work branch).** Implement the plan minimally. Verify any changed claim on an authoritative source *before* editing. Add or update tests — a behaviour-changing UI/feature/refactor ships **with** the regression test that locks it (that test is its verification in a headless run). Run the verify command until green (fix causes, never skip). Update the backlog entry. Commit with a conventional message (`feat:`/`fix:`/`refactor:`/`docs:`) **without** an AI signature. **Do not push** — the push waits for review approval.
